/**
 * 事件发射器类 - 用于处理事件监听和分发
 * Event Emitter Class - Handles event listening and dispatching
 */
let _l = class {
    /**
     * 构造函数 - 初始化事件存储数组
     * Constructor - Initialize events storage array
     */
    constructor() {
        this.events = []
    }
     ...
     * 添加事件监听器
     * Add event listener
     * @param {string} t - 事件类型 (event type)
     * @param {function} e - 回调函数 (callback function) 
     * @param {object} i - 上下文对象 (context object)
     * @param {boolean} s - 是否只执行一次 (execute only once)
     */
    on(t, e, i, s = !1) {
        ...
        this.events[t].forEach(n => {
            ...
            // 如果不存在则添加新的监听器
            // Add new listener if it doesn't exist
    ...
    emit(t) {
        ...
        this.events[t] && this.events[t].forEach((s, r) => {
            // 在指定上下文中执行回调函数
            // Execute callback in specified context
            s.cb.apply(s.context, i),
                // 如果是一次性事件，执行后删除
                // Remove if it's a one-time event
...
/**
 * 初始化检查函数 - 确保对象已被初始化
 * Initialization check function - Ensures object has been initialized
 */
function oi(u) {
    if (u === void 0)
...
/**
 * 原型继承函数 - 设置对象间的继承关系
 * Prototype inheritance function - Sets up inheritance between objects
 * @param {function} u - 子类构造函数 (child constructor)
 * @param {function} t - 父类构造函数 (parent constructor)
 */
function Ta(u, t) {
...
/*!
 * GSAP 3.12.5 - 专业级JavaScript动画库
 * GSAP 3.12.5 - Professional JavaScript Animation Library
 * https://gsap.com
 *
 * GSAP是一个功能强大的JavaScript动画库，用于创建高性能的Web动画
 * GSAP is a powerful JavaScript animation library for creating high-performance web animations
 *
...
/**
 * GSAP核心配置对象 - 定义全局动画设置
 * GSAP core configuration object - Defines global animation settings
 */
var Se = {
    autoSleep: 120,     // 自动休眠时间（秒）- Auto sleep time in seconds
    force3D: "auto",    // 强制3D加速设置 - Force 3D acceleration setting
    nullTargetWarn: 1,  // 空目标警告 - Null target warning
    units: {
        lineHeight: ""  // 行高单位设置 - Line height unit setting
    }
},
    /**
     * 动画默认设置 - 定义动画的默认参数
     * Animation default settings - Defines default animation parameters
     */
    Es = {
        duration: .5,       // 默认动画持续时间 - Default animation duration
        overwrite: !1,      // 是否覆盖现有动画 - Whether to overwrite existing animations
        delay: 0            // 默认延迟时间 - Default delay time
    },
    // 全局变量声明 - Global variable declarations
    ao,                     // 动画对象变量 - Animation object variable
    Xt,                     // 扩展变量 - Extension variable
    ct,                     // 计数器变量 - Counter variable
    Ae = 1e8,               // 极大数值常量 - Very large numeric constant
    ut = 1 / Ae,            // 极小数值常量 - Very small numeric constant
    Ln = Math.PI * 2,       // 2π常量 - 2π constant
    Dl = Ln / 4,            // π/2常量 - π/2 constant
    ml = 0,                 // 初始值常量 - Initial value constant
    Fa = Math.sqrt,         // 平方根函数 - Square root function
    yl = Math.cos,          // 余弦函数 - Cosine function
    vl = Math.sin,          // 正弦函数 - Sine function

    /**
     * 类型检测函数组 - 用于检测变量类型
     * Type detection functions - Used to detect variable types
     */
    zt = function (t) {
        return typeof t == "string"     // 检测是否为字符串 - Check if string
    },
    Dt = function (t) {
        return typeof t == "function"   // 检测是否为函数 - Check if function
    },
    fi = function (t) {
        return typeof t == "number"     // 检测是否为数字 - Check if number
    },
    uo = function (t) {
        return typeof t > "u"           // 检测是否未定义 - Check if undefined
    },
    ii = function (t) {
        return typeof t == "object"     // 检测是否为对象 - Check if object
    },
    he = function (t) {
        return t !== !1                 // 检测是否不为false - Check if not false
    },
    lo = function () {
        return typeof window < "u"      // 检测是否在浏览器环境 - Check if in browser environment
    },
    xr = function (t) {
        return Dt(t) || zt(t)          // 检测是否为函数或字符串 - Check if function or string
    },
    Pa = typeof ArrayBuffer == "function" && ArrayBuffer.isView || function () { }, // ArrayBuffer检测 - ArrayBuffer detection
    Gt = Array.isArray,    // 数组检测函数 - Array detection function

    /**
     * 正则表达式集合 - 用于解析和处理动画值
     * Regular expression collection - Used for parsing and processing animation values
     */
    On = /(?:-?\.?\d|\.)+/gi,          // 匹配数字 - Match numbers
    ka = /[-+=.]*\d+[.e\-+]*\d*[e\-+]*\d*/g, // 匹配科学计数法数字 - Match scientific notation numbers
    _s = /[-+=.]*\d+[.e-]*\d*[a-z%]*/g,      // 匹配带单位的数字 - Match numbers with units
    pn = /[-+=.]*\d+\.?\d*(?:e-|e\+)?\d*/gi, // 匹配浮点数 - Match floating point numbers
    Ma = /[+-]=-?[.\d]+/,                     // 匹配相对值 - Match relative values
    Aa = /[^,'"\[\]\s]+/gi,                   // 匹配非分隔符字符 - Match non-delimiter characters
    xl = /^[+\-=e\s\d]*\d+[.\d]*([a-z]*|%)\s*$/i, // 匹配完整数值表达式 - Match complete numeric expressions

    /**
     * 核心变量声明 - GSAP系统的核心变量
     * Core variable declarations - Core variables for GSAP system
     */
    pt,                     // 指针变量 - Pointer variable
    Ge,                     // 几何变量 - Geometry variable
    Rn,                     // 运行变量 - Run variable
    ho,                     // 持有者变量 - Holder variable
    Ee = {},                // 全局属性映射 - Global property mapping
    Xr = {},                // 扩展属性映射 - Extended property mapping
    La,                     // 最后动画变量 - Last animation variable

    /**
     * 工具函数集合 - GSAP系统的辅助函数
     * Utility function collection - Helper functions for GSAP system
     */
    Oa = function (t) {
        return (Xr = ts(t, Ee)) && oe   // 属性设置函数 - Property setting function
    },
    co = function (t, e) {
        return console.warn("Invalid property", t, "set to", e, "Missing plugin? gsap.registerPlugin()") // 无效属性警告 - Invalid property warning
    },
    ar = function (t, e) {
        return !e && console.warn(t)    // 条件警告函数 - Conditional warning function
    },
    Ra = function (t, e) {
        return t && (Ee[t] = e) && Xr && (Xr[t] = e) || Ee // 属性注册函数 - Property registration function
    },
    ur = function () {
        return 0                        // 返回零函数 - Return zero function
    },

    /**
     * 动画控制配置对象 - 用于控制动画行为
     * Animation control configuration objects - Used to control animation behavior
     */
    wl = {
        suppressEvents: !0,             // 抑制事件 - Suppress events
        isStart: !0,                   // 是否开始 - Is start
        kill: !1                       // 是否销毁 - Kill flag
    },
    zr = {
        suppressEvents: !0,             // 抑制事件 - Suppress events
        kill: !1                       // 是否销毁 - Kill flag
    ...
    /**
     * 系统数据存储对象 - GSAP系统的数据存储
     * System data storage objects - Data storage for GSAP system
     */
    fo = {},                           // 函数对象存储 - Function object storage
    Ti = [],                           // 时间轴数组 - Timeline array
    zn = {},                           // 区域数据 - Zone data
    za,                                // 缩放动画变量 - Zoom animation variable
    ye = {},                           // 年份数据 - Year data
    gn = {},                           // 组节点数据 - Group node data
    zo = 30,                           // 默认帧率 - Default frame rate
    Br = [],                           // 浏览器数组 - Browser array
    po = "",                           // 位置字符串 - Position string

    /**
     * 目标处理函数 - 处理动画目标对象
     * Target processing function - Processes animation target objects
     * @param {Array} t - 目标数组 - Target array
     * @returns {Array} 处理后的目标数组 - Processed target array
     */
    go = function (t) {
        var e = t[0], i, s;
        // 检查目标类型和处理器 - Check target type and handler
        if (ii(e) || Dt(e) || (t = [t]),
            !(i = (e._gsap || {}).harness)) {
            // 寻找合适的处理器 - Find appropriate handler
            for (s = Br.length; s-- && !Br[s].targetTest(e);)
                ;
            i = Br[s]
        }
        // 为每个目标创建GSAP实例 - Create GSAP instance for each target
        for (s = t.length; s--;)
    ...
    /**
     * GSAP实例获取函数 - 获取元素的GSAP实例
     * GSAP instance getter - Gets GSAP instance of element
     * @param {Element} t - 目标元素 - Target element
     * @returns {Object} GSAP实例 - GSAP instance
     */
    Xi = function (t) {
    ...
    /**
     * 属性值获取函数 - 获取元素的属性值
     * Attribute value getter - Gets attribute value of element
     * @param {Element} t - 目标元素 - Target element
     * @param {string} e - 属性名 - Attribute name
     * @param {*} i - 默认值 - Default value
     * @returns {*} 属性值 - Attribute value
     */
    Ba = function (t, e, i) {
    ...
    /**
     * 逗号分隔字符串处理函数 - 处理逗号分隔的字符串
     * Comma-separated string processor - Processes comma-separated strings
     * @param {string} t - 输入字符串 - Input string
     * @param {function} e - 处理函数 - Processing function
     * @returns {Array} 处理后的数组 - Processed array
     */
    ce = function (t, e) {
    ...
    /**
     * 数值精度修正函数（5位小数）- Number precision correction (5 decimal places)
     * @param {number} t - 输入数值 - Input number
     * @returns {number} 修正后的数值 - Corrected number
     */
    xt = function (t) {
    ...
    /**
     * 数值精度修正函数（7位小数）- Number precision correction (7 decimal places)
     * @param {number} t - 输入数值 - Input number
     * @returns {number} 修正后的数值 - Corrected number
     */
    Rt = function (t) {
    ...
    /**
     * 相对值计算函数 - 计算相对值
     * Relative value calculator - Calculates relative values
     * @param {number} t - 基础值 - Base value
     * @param {string} e - 相对值字符串 - Relative value string
     * @returns {number} 计算结果 - Calculation result
     */
    vs = function (t, e) {
        var i = e.charAt(0),        // 操作符 - Operator
            s = parseFloat(e.substr(2)); // 操作数 - Operand
        return t = parseFloat(t),
            i === "+" ? t + s : i === "-" ? t - s : i === "*" ? t * s : t / s
    },

    /**
     * 数组包含检查函数 - 检查数组是否包含特定值
     * Array contains checker - Checks if array contains specific values
     * @param {string} t - 查找字符串 - Search string
     * @param {Array} e - 目标数组 - Target array
     * @returns {boolean} 是否包含 - Whether contains
     */
    Cl = function (t, e) {
    ...
    /**
     * 延迟渲染处理函数 - 处理延迟渲染的动画
     * Lazy render processor - Processes lazy-rendered animations
     */
    Gr = function () {
        var t = Ti.length, e = Ti.slice(0), i, s;
        // 清空时间轴数组和区域数据 - Clear timeline array and zone data
        for (zn = {},
            Ti.length = 0,
            i = 0; i < t; i++)
            s = e[i],
                // 执行延迟渲染 - Execute lazy rendering
                s && s._lazy && (s.render(s._lazy[0], s._lazy[1], !0)._lazy = 0)
    },

    /**
     * 动画渲染函数 - 渲染动画
     * Animation renderer - Renders animations
     * @param {Object} t - 动画对象 - Animation object
     * @param {number} e - 时间 - Time
     * @param {boolean} i - 是否抑制事件 - Suppress events
     * @param {*} s - 额外参数 - Additional parameter
     */
    Ia = function (t, e, i, s) {
        // 处理延迟渲染队列 - Process lazy render queue
        Ti.length && !Xt && Gr(),
            // 执行动画渲染 - Execute animation rendering
            t.render(e, i, Xt && e < 0 && (t._initted || t._startAt)),
            // 再次处理延迟渲染队列 - Process lazy render queue again
            Ti.length && !Xt && Gr()
    },

    /**
     * 数值解析函数 - 解析数值
     * Number parser - Parses numbers
     * @param {*} t - 输入值 - Input value
     * @returns {number|string} 解析结果 - Parsed result
     */
    Na = function (t) {
        var e = parseFloat(t);
    ...
    /**
     * 恒等函数 - 返回输入值
     * Identity function - Returns input value
     * @param {*} t - 输入值 - Input value
     * @returns {*} 相同的值 - Same value
     */
    Wa = function (t) {
        return t
    },
    ...
    /**
     * 对象属性合并函数 - 合并对象属性（不覆盖现有属性）
     * Object property merger - Merges object properties (doesn't overwrite existing)
     * @param {Object} t - 目标对象 - Target object
     * @param {Object} e - 源对象 - Source object
    ...
    /**
     * 选择性属性设置器工厂 - 创建选择性属性设置函数
     * Selective property setter factory - Creates selective property setter
     * @param {boolean} t - 是否为关键帧模式 - Whether keyframe mode
     * @returns {function} 属性设置函数 - Property setter function
     */
    Sl = function (t) {
        return function (e, i) {
            for (var s in i)
    ...
    /**
     * 对象属性复制函数 - 复制对象的所有属性
     * Object property copier - Copies all object properties
     * @param {Object} t - 目标对象 - Target object
     * @param {Object} e - 源对象 - Source object
     * @returns {Object} 目标对象 - Target object
     */
    ts = function (t, e) {
        for (var i in e)
            t[i] = e[i];
    ...
    /**
     * 深度对象合并函数 - 深度合并对象
     * Deep object merger - Deep merges objects
     * @param {Object} t - 目标对象 - Target object
     * @param {Object} e - 源对象 - Source object
     * @returns {Object} 合并后的对象 - Merged object
     */
    ...
    /**
     * 对象差异提取函数 - 提取两个对象的差异
     * Object difference extractor - Extracts differences between objects
     * @param {Object} t - 第一个对象 - First object
     * @param {Object} e - 第二个对象 - Second object
    ...
    /**
     * 继承设置函数 - 设置对象继承关系
     * Inheritance setter - Sets object inheritance
     * @param {Object} t - 子对象 - Child object
     * @returns {Object} 处理后的对象 - Processed object
     */
    Gs = function (t) {
        var e = t.parent || pt,  // 父对象 - Parent object
            i = t.keyframes ? Sl(Gt(t.keyframes)) : Re; // 属性设置函数 - Property setter
        if (he(t.inherit))
            for (; e;)
                i(t, e.vars.defaults),
                    e = e.parent || e._dp;
